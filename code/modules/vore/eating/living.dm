...
/mob/living/proc/lick()
	...
	// BLUEMOON EDIT START
	var/mob/living/tasted
	if(lickable.len == 1)
		tasted = lickable[1]
	else
		tasted = show_radial_menu(src, src, lickable, radius = 40, require_near = TRUE)
	// BLUEMOON EDIT END
...
/mob/living/proc/smell_someone()
	...
	// BLUEMOON EDIT START
	var/mob/living/sniffed
	if(smellable.len == 1)
		sniffed = smellable[1]
	else
		sniffed = show_radial_menu(src, src, smellable, radius = 40, require_near = TRUE)
	// BLUEMOON EDIT END
