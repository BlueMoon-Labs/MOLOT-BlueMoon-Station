/obj/item/clothing/shoes
	...
	var/list/bloody_shoes = list(BLOOD_STATE_BLOOD = 0, BLOOD_STATE_OIL = 0, BLOOD_STATE_NOT_BLOODY = 0, BLOOD_STATE_POO = 0)
	var/offset = 0
...
/obj/item/clothing/shoes/clean_blood(datum/source, strength)
	. = ..()
	bloody_shoes = list(BLOOD_STATE_BLOOD = 0, BLOOD_STATE_OIL = 0, BLOOD_STATE_NOT_BLOODY = 0, BLOOD_STATE_POO = 0)
...
/obj/item/clothing/shoes/proc/handle_tying(mob/user)
	...
	if(user == loc && tied != SHOES_TIED) // if they're our own shoes, go tie-wards
		if(INTERACTING_WITH(user, our_guy))
			...
			return
		user.visible_message("<span class='notice'>[user] begins [tied ? "unknotting" : "tying"] the laces of [user.ru_ego()] [src.name].</span>", "<span class='notice'>You begin [tied ? "unknotting" : "tying"] the laces of your [src.name]...</span>")
