...
/mob/living/proc/do_oral_self(mob/living/user, var/fucktarget = "penis")
	...
	if(fucktarget == "vagina")
		...
		if(user.can_penetrating_genital_cum())
			...
			user_strapon.attached_dildo.target_reaction(user, null, 1, CUM_TARGET_MOUTH, null, FALSE, FALSE)
	//BLUEMOON EDIT END
...
/mob/living/proc/do_breastfuck_self(mob/living/user)
	...
	if(is_fucking(user, CUM_TARGET_BREASTS))
		...
		//BLUEMOON EDIT START
			"трёт свой "+(has_penis ? "член" : "дилдо")+" между своих сисек.",
...
/mob/living/proc/do_nipfuck(mob/living/target)
	...
	if(is_fucking(target, CUM_TARGET_NIPPLE) && target.has_breasts() == HAS_EXPOSED_GENITAL)
		lines = list(
			...
			"шлепает своими "+(has_balls ? "семянниками" : "бедрами")+" по груди <b>[target]</b>, как в то же время сосок прогладывает всю длинну "+(has_penis ? "члена" : "дилдо")+"."
		)
