...
/obj/effect/mob_spawn/human/inteqspace/captain
	...
	flavour_text = "Ваш корабль причалил после долгого перерыва где-то во враждебном пространстве, сообщив о неисправности. Вы застряли здесь, зная, что рядом находится станция Nanotrasen. Командуйте своим экипажем и исследуйте сво территорию, чтобы закрепить свое местоположение."
	important_info = "Установите полную власть над локальным сектором, защитите корабль и секретные документы в рюкзаке ценой своей жизни."
	can_load_appearance = TRUE
