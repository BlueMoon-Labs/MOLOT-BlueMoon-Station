...
/datum/station_goal/bfl/get_report()
	...
	добычи плазмы и отправки её на ЦК через шаттл карго. Благодаря нашему дрону точная геопозиция жилы доступна на GPS устройствах.
	<br>
	Базовые компоненты конструкции должны быть доступны для покупки через шаттл карго.
	<br>
	Конструкция состоит из трёх компонентов: Излучателя, Приемника и Линзы. Излучатель устанавливается на станции и требует прямого подключения к \
	энергосети, Приемник сооружается прямо над центром жилы, а Линза устанавливается на Приемник. Створки Приемника должны быть открыты. \
	Если конструкция установлена и настроена правильно, Излучатель сможет направлять лазер точно в Приемник и тот будет собирать добытую лазером \
	руду, в противном случае возможно незначительное смещение лазера относительно местонахождения жилы.
	<br>
