...
/datum/station_goal/bfl/get_report()
	...
	Наш разведывательный дрон обнаружил на астероиде огромную жилу, изобилующей плазмой. Нам нужно, чтобы вы построили BFL систему для добычи плазмы и отправки её на ЦК через шаттл карго. \
	Благодаря нашему дрону точная геопозиция жилы доступна на GPS устройствах.
	<br>
	Базовые компоненты конструкции должны быть доступны для покупки через шаттл карго.
	<br>
	Конструкция состоит из трёх компонентов: Излучателя, Приемника и Линзы. Излучатель устанавливается на станции и требует прямого подключения к сети станции, Приемник сооружается прямо над центром жилы, а \
	Линза устанавливается на Приемник. Створки Приемника должны быть открыты. Если конструкция установлена и настроена правильно, Излучатель сможет \
