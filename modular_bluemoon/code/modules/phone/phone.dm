	...
	if(inbound_call)
		...
		if(attached_to.loc == src)
			if(next_ring < world.time)
				playsound(loc, 'modular_bluemoon/sound/machines/telephone/telephone_ring.ogg', 75)
...
/obj/structure/transmitter/proc/handle_speak(message, datum/language/L, mob/speaking)
	...
	playsound(P, pick('modular_bluemoon/sound/machines/telephone/talk_phone1.ogg',
						...
						'modular_bluemoon/sound/machines/telephone/talk_phone7.ogg'), 5)
...
/obj/item/telephone/proc/handle_speak(mob/speaking, message, datum/language/L)
	SIGNAL_HANDLER
...
/obj/item/telephone/proc/handle_hear(message, datum/language/L, mob/speaking)
	if(!attached_to)
		return
...
/obj/item/telephone/attack_hand(mob/user)
...
/obj/item/telephone/pickup(mob/user)
	. = ..()
	flags_1 = CONDUCT_1 | HEAR_1
	// RegisterSignal(user, COMSIG_LIVING_SPEAK, PROC_REF(handle_speak))

/obj/item/telephone/forceMove(atom/dest)
	. = ..()
	// if(.)
	// 	reset_tether()
