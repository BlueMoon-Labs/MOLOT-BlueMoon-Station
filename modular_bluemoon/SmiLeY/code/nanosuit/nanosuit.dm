...
/obj/item/clothing/glasses/nano_goggles
	...
	mob_overlay_icon = 'modular_bluemoon/SmiLeY/icons/nanosuit/nanosuit_mob.dmi'
	icon = 'modular_bluemoon/SmiLeY/icons/nanosuit/nanosuit.dmi'
...
/obj/item/clothing/suit/space/hardsuit/nano/run_block(mob/living/carbon/human/user, atom/movable/hitby, attack_text = "атаку", final_block_chance = 0, damage = 0, attack_type = ATTACK_TYPE_MELEE)
	...
	if(mode == NANO_ARMOR && cell && cell.charge)
		if(prob(final_block_chance))
			...
			if(damage)
				...
				else if(P.damage_type == STAMINA && attack_type == ATTACK_TYPE_PROJECTILE)
...
/datum/action/item_action/nanosuit/goggletoggle
	name = "Night Vision"
	button_icon = 'modular_bluemoon/SmiLeY/icons/nanosuit/actions_nanosuit.dmi'
...
/obj/item/clothing/suit/space/hardsuit/nano/equipped(mob/user, slot)
	...
	if(slot == ITEM_SLOT_OCLOTHING)
		...
		if(is_station_level(T.z))
			priority_announce("[user] использовал[user.ru_a()] запрещённый нанокостюм в [A.name]!", "Экстренное сообщение!", sound('modular_bluemoon/SmiLeY/sounds/nanosuitengage.ogg'))
...
/datum/martial_art/nanosuit/proc/HeadStomp(mob/living/carbon/human/A, mob/living/carbon/human/D)
	...
	if(head)
		...
		playsound(get_turf(A), 'modular_bluemoon/SmiLeY/sounds/squishy.ogg', 75, TRUE, -1)
...
/mob/living/simple_animal/attack_hand(mob/user)
	. = ..()
	var/mob/living/carbon/human/M = ishuman(user) ? user : null
	if(M && istype(M.wear_suit, /obj/item/clothing/suit/space/hardsuit/nano))
...
/obj/item/clothing/suit/space/hardsuit/nano/proc/open_mode_menu(mob/living/user)
	...
	"armor" = image(icon = 'modular_bluemoon/SmiLeY/icons/nanosuit/actions_nanosuit.dmi', icon_state = "armor_menu"),
	"speed" = image(icon = 'modular_bluemoon/SmiLeY/icons/nanosuit/actions_nanosuit.dmi', icon_state = "speed_menu"),
	"cloak" = image(icon = 'modular_bluemoon/SmiLeY/icons/nanosuit/actions_nanosuit.dmi', icon_state = "cloak_menu"),
	"strength" = image(icon = 'modular_bluemoon/SmiLeY/icons/nanosuit/actions_nanosuit.dmi', icon_state = "strength_menu")
	)
